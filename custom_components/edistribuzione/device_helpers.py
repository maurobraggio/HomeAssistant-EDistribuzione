...
identificatori) in favore di via_device_id (l'ID interno del device padre),
perché gli identificatori non sono più univoci a livello globale ma solo per
config entry.

Non è solo una deprecazione con scadenza lontana (2027.8): su Home Assistant
2026.9 il platform sensor può trasformarla in un RuntimeError invece che in
un warning, quando la catena di chiamate non permette a Core di attribuire la
chiamata all'integrazione custom - in quel caso le entità NON vengono
aggiunte affatto (già successo ad altre integrazioni con HA 2026.9).

via_device_id però non esiste prima di HA 2026.8: si scelgono quindi a
runtime in base alla versione di Home Assistant.
"""
...
def supporta_via_device_id() -> bool:
    ...
    In caso di dubbio (versione non interpretabile) ritorna False: si ricade
    su via_device, che sulle versioni vecchie è l'unica cosa che funziona, e
    su quelle nuove produce al massimo un warning.
    """
    try:
...
def assicura_dispositivo_padre(
    ...
    Va chiamata PRIMA di costruire le entità figlie: al primo avvio il padre
    non esiste ancora nel registry, perché normalmente lo creano le entità
    stesse quando vengono aggiunte - quindi cercarlo e basta ritornerebbe
    None proprio quando serve.
    ...
    riesce: un fallimento qui non deve impedire la creazione delle entità,
    al massimo si perde il collegamento gerarchico nella UI.
    """
